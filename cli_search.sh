...
def get_db_connection():
    ...
    if not os.path.exists(index_path):
        ...
        print(f"{Colors.WHITE}Please ensure 'database/*.db' directory is placed in the SAME folder as the script.{Colors.RESET}")
        sys.exit(1)
...
def print_detailed_help(lang="en"):
    ...
    if lang == "ko":
        ...
        print(f"   ./cli_search.sh <검색어_또는_주소> [옵션...]\n")
        ...
        print(f"      ./cli_search.sh CameraCapturedBlurImage --lang ko")
        print(f"   {Colors.WHITE}2. 가상 주소 검색:{Colors.RESET}")
        print(f"      ./cli_search.sh 0x0417BE60")
        print(f"   {Colors.WHITE}3. 한국어 커맨드 인자 사용:{Colors.RESET}")
        print(f"      ./cli_search.sh --검색어 NetworkManager --언어 ko")
        print(f"   {Colors.WHITE}4. AI 주입용 JSON 형식 출력:{Colors.RESET}")
        print(f"      ./cli_search.sh NetworkManager --json\n")
    else:
        ...
        print(f"   ./cli_search.sh <Query_Keyword_or_Address> [Options...]\n")
